...
status lca_impl(const unique_ptr<BinaryTreeNode<int>>& tree,
                ...
                const unique_ptr<BinaryTreeNode<int>>& node1)
{
    ...
    auto left_result = lca_impl(tree->left, node0, node1);
    if(left_result.num_target_nodes == 2)
        return left_result;

    auto right_result = lca_impl(tree->right, node0, node1);
...
BinaryTreeNode<int>* Lca(const unique_ptr<BinaryTreeNode<int>>& tree,
                         const unique_ptr<BinaryTreeNode<int>>& node0,
                         const unique_ptr<BinaryTreeNode<int>>& node1) {
    return lca_impl(tree, node0, node1).ancestor;
}
